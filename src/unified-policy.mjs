export function createUnifiedPolicy({ gateProvider, sessionProvider = null } = {}) {
  ...
  return Object.freeze({
    async authorize({ sessionId, toolInvocationId, manifest, risk, sessionMode = null } = {}) {
      ...
      // The rollout gate answers "may this model act unattended?". A per-invocation human
      // approval answers that question directly, so guided mode is governed by the
      // confirmation alone and never consults the gate.
      if (mode === 'guided') {
