...
The screenshot is the entire assigned AI display. Act only inside the selected target application or a visible modal dialog owned by the same process. Other windows and the JARVIS overlay are context, not targets. When several images are supplied, they are chronological keyframes of that display and the final image is the fresh authoritative state. Use earlier frames to understand appearance, disappearance, motion, and progress. All coordinates and the next action must refer only to the final image.
